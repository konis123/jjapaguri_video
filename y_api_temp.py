...
def makeResultFile(list, p1_avgSpeed, p2_avgSpeed, p1_maxSpeed, p2_maxSpeed, winner):
    ...
    # if a_player_total_attack_num != 0:
    ...
    # else:
    #     print('a_player_total_attack_num: 0')
    #
    # if b_player_total_attack_num != 0:
    ...
    # else:
    #     print('b_player_total_attack_num: 0')
    #
    #
    # f.close()
    ...
    # velocity = []
    # for val in list:
    #     section.append(str(val[0]))
    #     time.append(int(val[1]))
    #     velocity.append(str(val[2]))
    ...
    #         'time': time,
    #         'velocity': velocity,
    #         'p1_avg_vel': p1_avgSpeed,
    #         'p2_avg_vel': p2_avgSpeed
    #         }
...
def anal(video):
    ...
    def initTracker(img, flag):  # Tracker가 Update 안 되었을 때만 flag가 1로 와서 동작
        ...
        for i in range(0, img.shape[0]):
            for j in range(0, img.shape[1]):
                ...
                if (img.shape[0] / 2) - sensivity <= i <= (img.shape[0] / 2) + sensivity:
                    img_black[i, j] = 0
                if 0 <= j <= 5 or (img.shape[1] - 5) <= j <= img.shape[1]:
                    img_black[i, j] = 0
        ...
        circles = cv2.HoughCircles(color, cv2.HOUGH_GRADIENT, 1, 1, param1=50, param2=1, minRadius=0, maxRadius=5)
    ...
    while True:  # 계속 잘 읽어오면
        ...
        # 'q' 누르면 종료
        if cv2.waitKey(200) == ord('q'):  # 1. 실제 화면을 볼 수 있도록 10ms씩 딜레이를 줌  2. 그 중 q를 누르면 종료가 됨
